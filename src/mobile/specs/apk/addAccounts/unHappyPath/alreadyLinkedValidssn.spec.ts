...
describe(`App - Android - Already linked account & Valid SSN link error scenario - ${language}`, () => {
  it(`Linked accounts- already linked accounts & Valid SSN link scenario- ${language}`, async () => {
    allureReporter.addLabel('Login TestCase', `Linked accounts- already linked accounts & Valid SSN link  scenario- ${language}`);
    allureReporter.addTestId('SDCARE-3120');
    allureReporter.addTestId('SDCARE-2490');
   ...
   await driver.eyesCheck(`${language} - account number field inline error validation`, Target.window());
    allureReporter.addTestId('SDCARE-2480');
    await (await myaccounts.accountNumberField).clearValue();
    await (await myaccounts.accountNumberField).click();
    driver.keys(accountDetails.validAccountNumber2);
    await (await myaccounts.hintBtn).click();
   ...
   await driver.eyesCheck(`${language} - Verify your identiy page default ssn field enabled`, Target.window());
    await (await myaccounts.phoneNumberToggleBtn).click();
   await driver.eyesCheck(`${language} - Verify your identiy page switch to phone number`, Target.window());
    await (await myaccounts.ssnNumberToggleBtn).click();
   await driver.eyesCheck(`${language} - Verify your identiy page switch to ssn number`, Target.window());
   ...
   await driver.eyesCheck(`${language} - validate the error description - Already linked error`, Target.window());
