...
['English', 'Spanish'].forEach((language) => {
  describe(`App - Android - Delete Account- unHappy Path under support - Menu - ${language}`, () => {
    it(`Delete account unhappy path under support - Menu - ${language}`, async () => {
      allureReporter.addLabel('Menu', `Delete account unhappy path screen under support - Menu - ${language}`);
      ...
      await (await menuScreen.deleteAccountTryAgainBtn).click();
      await driver.eyesCheck(`${language} - navigates back to delete account screen`, Target.window());
